...
jint start_native_library(JNIEnv *env, jobjectArray args, bool pegas) {
    //argc
    ...
    //Start library, with argc and argv.
    int result;
    if (pegas) {
        result = start_pegas(argument_count, argv);
    } else {
        result = start_tun2socks(argument_count, argv);
    };
    free(args_buffer);
...
Java_com_mokhtarabadi_pegasocks_MainNative__1native_1start_1tun2socks(JNIEnv *env, jclass clazz,
                                                                      jobjectArray args) {
    return start_native_library(env, args, false);
}

extern "C"
JNIEXPORT void JNICALL
Java_com_mokhtarabadi_pegasocks_MainNative__1native_1terminate_1tun2socks(JNIEnv *env,
                                                                          jclass clazz) {
    terminate();
}
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_mokhtarabadi_pegasocks_MainNative__1native_1start_1pegas(JNIEnv *env, jclass clazz,
                                                                  jobjectArray args) {
    return start_native_library(env, args, true);
}
